...
self.addEventListener('install', e => {
    const staticCache = caches.open(STATIC_CACHE).then(
        cache => cache.addAll(APP_SHELL));
    const inmutableCache = caches.open(INMUTABLE_CACHE).then(
    ...
    const response = caches.match(e.request).then(res => {
        ...
        if (res) {
            ...
            return fetch(e.request).then(
                newRes => {
                    return dynamicUpdate(DYNAMIC_CACHE, e.request, newRes);
                });
        }
